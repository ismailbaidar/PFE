...
export const deleteBrand = createAsyncThunk("deleteBrand", async (data) => {
    return axios
        .delete("http://localhost:8000/api/brand/" + data)
        .then((res) => res.data)
        .catch((err) => err);
});
...
export const addBrand = createAsyncThunk("addBrand", async (data) => {
    ...
    return axios
        .post("http://localhost:8000/api/brand", form)
        .then((res) => res.data)
        .catch((err) => err);
});
...
export const getOne = createAsyncThunk("GetOne", async (id) => {
    return axios
        .get("http://localhost:8000/api/brand/" + id)
        .then((res) => res.data)
        .catch((err) => err);
});
export const updateBrand = createAsyncThunk("UpdateBrand", async (data) => {
    ...
    form.append("img", data.file);
    return axios
        .post("http://localhost:8000/api/brand/" + data.id, form)
        .then((res) => res.data)
        .catch((err) => err);
});
...
const brandSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder.addCase(getBrands.fulfilled, (state, { payload }) => {
            console.log("brands slice", payload);
