...
export const checkoutWithCardPaymant=createAsyncThunk('checkoutWithCardPaymant',async(data,{getState})=>{
        ...
        form.append('products',JSON.stringify([{id:2,qte:4},{id:3,qte:5}]))
        return axios.post('http://localhost:8000/api/checkout',form)
        .then(res=>res.data)
        .then(res=>window.location.href=res.url)
        .catch(err=>console.log(err))
})

export const checkoutLivresion=createAsyncThunk('checkoutLivresion',async(data,{getState})=>{
...
const CheckoutSlice=createSlice({
    name:'CheckoutSlice',
    initialState:{status:'',data:{name:'',prenom:'',adress:'',postal:'',ville:''}},
    reducers:{
    ...
    extraReducers:(builder)=>{
        builder.addCase(checkoutWithCardPaymant.fulfilled,(state,{payload})=>{
            state.status=payload.status
        })
        builder.addCase(checkoutLivresion.fulfilled,(state,{payload})=>{
        })
        builder.addCase(checkoutWithCardPaymant.rejected,(state,{payload})=>{
            console.log(payload)
        })
    }
)
