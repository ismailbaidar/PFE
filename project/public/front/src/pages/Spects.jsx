...
import {deleteSpect,getSpects} from '../features/SpectSlice'
...
const Brands=()=>{
    ...
    const status = useSelector(state=>state.Spect.status)
    console.log(status)
    const [success,setSuccess]=useState(null);
    const Rows = useSelector(state=>state.Spect.spects)
    useEffect(()=>{
        dispatch(getSpects())
    },[])
    const deleteC=()=>{
        dispatch(deleteSpect(Delete))
        setDeleted(null)
        dispatch(getSpects())
    }
    useEffect(()=>{
    ...
    const columns = [
            ...
            renderCell: (params) => {
                return (
                    <div className="actionbtntable">
                        ...
                        <button
                            onClick={(e) => {e.stopPropagation(); location('/admin/OptionModifier/'+params.id)}}
                            className="edite"
        ...
        return (
            ...
            <button  className="ajouterProduit" onClick={()=>location('/admin/ajouteroption')} >Ajouter Brand</button>
            <DataGrid
            rows={Rows}
            columns={columns}
