...
const Categories = () => {
    ...
    const Rows = useMemo(()=>{
      return  categories.map(e=>({name:e.name,id:e.id,categorie_id:e.parent?.name}))
    })
            ...
            renderCell: (params) => {
                return (
                    <div className="actionbtntable">
                        ...
                        <button
                            onClick={(e) => {e.stopPropagation(); location('/admin/Categoriemodifier/'+params.id)}}
                            className="edite"
    ...
    return (
        <div className="ProductsDashboard">
            <AnalyseProduct />
            <button  className="ajouterProduit" >Ajouter Categorie</button>
            <DataGrid
                rows={Rows}
                columns={columns}
